 ...
 * 首页tab操作服务
 */
services.factory('TabOperator',['$rootScope','$location','$timeout',function($rootScope,$location,$timeout){
	//当前显示的所有tab
	$rootScope.tabs = [];
	//当前隐藏的所有tab
	$rootScope.hiddenTabs = [];
    //保持tab激活顺序的队列
	var activeQueue=[];
     ...
     */
    function initMenuStatus(menus,matchUrl){
    	
    	//置空旧状态
        if($rootScope.menuPath&&$rootScope.menuPath.length>0){
        	 for (var i = 0; i < $rootScope.menuPath.length; i++) {
        		 $rootScope.menuPath[i].menu.active=false;
        	 }
        }
        
    	//获取新的状态
        initMenuPath(menus,matchUrl);
    	if($rootScope.menuPath&&$rootScope.menuPath.length>0){
    		 for (var i = 0; i < $rootScope.menuPath.length; i++) {
    			 $rootScope.menuPath[i].menu.active=true;
        	 }
    	}
    }
     ...
     * @param menus  子menus数组
     */
    function initMenuPath(menus,matchUrl){
        ...
        for (var i = 0; i < menus.length; i++) {
            var menu = menus[i];
            if (menu) {
                if (menu.l && menu.l ==matchUrl){
                	$rootScope.menuPath.push({title:menu.t,menu:menu,url:menu.l});
                    return true;
                }
                if (menu.c && menu.c.length > 0) {
                    var flag=initMenuPath(menu.c,matchUrl);
                    if(flag){
                    	$rootScope.menuPath.push({title:menu.t,menu:menu,url:menu.l});
                    	 return true;
                    }
     ...
     * 递归查找菜单
     */
	var findMenu=function(url,menus) {
		var fmenu;
        ...
        for (var i = 0; i < menus.length; i++) {
            ...
            if (menu){
                if (menu.l && (menu.l ==url||menu.l=="#"+"/"+url)){
                	fmenu=menu;
                	break;
                }
                if (menu.c && menu.c.length > 0){
                	fmenu=findMenu(url,menu.c);
                	if(!fmenu){
                		continue;
                	}else{
                		break;
	...
	var removeFromActiveQueue=function(tab){
		 var index = activeQueue.indexOf(tab);
	     if(index != -1){
	    	//挪到最后
	...
	//根据menu配置创建tab对象
	var createTab=function(openUrl,menu){
		return {
				title:menu.t,
				content:menu.t,
				page:openUrl,
	...
	var instance={
			setNavBarWidth:function(width){
				config.style.totalWidth=width;
				config.style.maxVisibleTab = Math.floor((width-config.style.foldBtnWidth)/config.style.tabMinWidth) - 1;
			},
			openTab:function(data,addToActiveQueueHead){
				//data可能是tab对象，也可能新页面的url
				//addToActiveQueueHead控制是将新的页面添加到激活队列最后还是最前，最前即第一个被自动移除的tab
				var currTab,openUrl=undefined;
				
				if (typeof (data) !== "string"){
					currTab = data;
					openUrl=currTab.page;
		        } else {
		        	openUrl = data;
		        }
	    	    ...
	    	    if(openUrl.indexOf("/")==0){
	    	    	openUrl=openUrl.substr(1,openUrl.length);
	    	    }
	    	    
	    	    //当菜单还未加载完成
	    		if(angular.isUndefined($rootScope.menus)||$rootScope.menus.length<=0){
	    			waitUrl=openUrl;
	    			return;
	    		}
				...
				for(var i=0;i<$rootScope.tabs.length;i++){
				  if ($rootScope.tabs[i].page == openUrl){
					  $rootScope.tabs[i].active=true;
					  initMenuStatus($rootScope.menus,window.location.hash);
				      return true;
				  }
			    }
				//处理菜单URL中有中文或URL无法传输的编码时，找不到匹配URL的情况
		  		var decodedUrl=decodeURIComponent(openUrl);
				var fmenu=findMenu(decodedUrl,$rootScope.menus);
				if(fmenu){
					currTab=createTab(openUrl,fmenu);
				}else{
					if(!currTab){
						currTab=createTab(openUrl,{t:""});
					}
				}
				
	  	        //tab打开前置接口
	  		    if(angular.isFunction(config.events.beforeOpen)){
	  			   var re=config.events.beforeOpen(openUrl,currTab);
	  			   //是否中止tab页创建
	  		    ...
	  		    if(addToActiveQueueHead){
	  		    	//是从隐藏状态恢复为可见状态，不应该激活
	  		    	currTab.active=false;
	  		    }
				$rootScope.tabs.push(currTab);
				if(addToActiveQueueHead){
					activeQueue.unshift(currTab);
				}else{
					activeQueue.push(currTab)
				}
				
				initMenuStatus($rootScope.menus,window.location.hash);
				
		  	    //tab打开后置接口
		  		if(angular.isFunction(config.events.afterOpen)){
		  			config.events.afterOpen(openUrl,currTab);
		  		}
			},
			showHiddenTab:function(tab){
				var index = $rootScope.hiddenTabs.indexOf(tab);
		  		if(index!=-1){
		  			 $rootScope.hiddenTabs.splice(index,1);
		  		}
		  		this.openTab(tab,true);
			},
			adjust:function(){
				var navbarDom=$(".tabbable>div>ul");

				var maxWidth=navbarDom.width() - config.style.foldBtnWidth;
				
				var totalWidth=0;
				
				navbarDom.children("li").each(function(index,li){
					totalWidth+=$(li).width();
				});
			    ...
			    if(totalWidth > maxWidth){
			    	do{
			    		//超过能显示的总数了
			    		var oldestTab=activeQueue.shift();
			    		var index = $rootScope.tabs.indexOf(oldestTab);
			    		if(index!=-1){
			    			//不调用close，避免触发关闭事件
			    			$rootScope.tabs.splice(index,1);
			    			navbarDom.children("li").each(function(index,li){
								if($(li).scope().tab==oldestTab){
									totalWidth-=$(li).width();
								}
							});
			    		}
			    		$rootScope.hiddenTabs.push(oldestTab);
			    	}while(totalWidth > maxWidth);
		    	}else if($rootScope.hiddenTabs.length>0){
		    		var _this=this;
		    		$timeout(function(){
		    			//有空间，尝试将隐藏的Tab显示出来
			    		var hiddenTab=$rootScope.hiddenTabs.pop();
			    		//tab在被添加回去之后会触发隐藏的重新计算，第二个参数为true，则在导航超长后会立即将这个tab隐藏掉
			    		_this.openTab(hiddenTab,true);
		    		},0);
		    	}
			},
			pinTab:function(target){
				if(angular.isDefined(target.$pin)){
					target.$pin=undefined;
				}else{
					target.$pin=pinCounter;
					pinCounter++;
					//pin住的标签不参与active运算
					removeFromActiveQueue(target);
				}
			},
			closeTab:function(target){
				var index=$rootScope.tabs.indexOf(target);
				$rootScope.tabs.splice(index,1);
		  	    //tab打开后置接口
		  		if(angular.isFunction(config.events.onClose)){
		  			config.events.onClose(target);
		  		}
		  		removeFromActiveQueue(target);
		  		
		  		if($rootScope.hiddenTabs.length>0){
		  			this.adjust();
		  		}
			},
			selectTab:function(url,tab){
		    	...
		    	//window.location.hash=url;
	  	        ...
	  	        //tab选中接口
	  		    if(angular.isFunction(config.events.onSelect)){
	  		    	config.events.onSelect(url,tab);
	  		    }
	  		    
	  		    if(angular.isDefined(tab)){
	  		    	 if(angular.isDefined(tab.$pin)){
	 		   			//pin住的标签不参与active运算
	 	  		    	return;
	 		   		}
	 	  		    removeFromActiveQueue(tab);
	 	     	    activeQueue.push(tab);
	  		    }
			},
			getWaitUrl:function(){
				return waitUrl;
			},
			setWaitUrl:function(url){
				waitUrl=url;
			},
			getCurrentTab:function(){
				if($rootScope.tabs==undefined) return undefined;
				for(var i=0;i<$rootScope.tabs.length;i++){
				  if ($rootScope.tabs[i].active == true){
			...
			getAllTab:function(){
				return $rootScope.tabs;
			},
			refreshTab:function(url){
				var tb=this.getTab(url);
				
				if(!tb) return;
				
				if(url.indexOf("?")!=-1){
					tb.includeUrl=url+"&_t="+new Date().getTime();
				}else{
					tb.includeUrl=url+"?_t="+new Date().getTime();
				}
			},
...
services.factory('TitleResponseInterceptor', [ '$q', 'Messenger','$rootScope',
       function($q, Messenger,$rootScope) {
   			return {
   				'response' : function(response) {
   					var tabtitle=response.headers()['form-title'];
   					if(tabtitle!=undefined&&tabtitle!=""){
   						tabtitle=decodeURIComponent(response.headers()['form-title']./*将编码后的加号处理成空格*/replace(/\+/g, '%20'));
   						
   						//先在已打开列表中查找tab页
					    for(var i=0;i<$rootScope.tabs.length;i++){
						  if ($rootScope.tabs[i].page == response.config.url){
							  if($rootScope.tabs[i].title==undefined||$rootScope.tabs[i].title==""){
								  $rootScope.tabs[i].title=tabtitle;
							  }
						  }
					    }
   					}
...
directives.run(["$templateCache", function($templateCache) {
	  $templateCache.put("template/tabs/c2tabset.html",
	    "\n" +
	    "<div>\n" +
	    "  <ul class=\"nav nav-tabs tab-fixed \" style=\"paddin-top: 0px;\" ng-class=\"{'nav-stacked': vertical, 'nav-justified': justified}\" ng-transclude></ul>\n" +
	    "  <div class=\"tab-content nav-tab-border\" style=\"border: 0px;padding-top: 40px;padding-right:0px;padding-left:0px;padding-bottom: 0px;\">\n" +
	    "    <div class=\"tab-pane\" \n" +
	    "         ng-repeat=\"tab in tabs\" \n" +
	    "         ng-class=\"{active: tab.active}\"\n" +
	    "         c2tab-content-transclude=\"tab\">\n" +
	    "    </div>\n" +
	    "  </div>\n" +
	    "</div>\n" +
	    "");
	  
	  $templateCache.put("template/tabs/c2tab.html",
			    "<li class=\"animate-repeat\" ng-class=\"{active: active, disabled: disabled}\" ng-click=\"select()\" >" +
			    "  <a  href=\"javascript:void(0);\" style=\"height: 40px;margin-top: 0px;\" c2tab-heading-transclude></a>\n" +
			    "</li>" 
			  );
	}]);

directives.directive('c2tabset', ['$rootScope',function($rootScope) {
	  return {
		    restrict: 'EA',
		    transclude: true,
		    replace: true,
		    ...
		    controller: function TabsetCtrl($scope,$element) {
		    	 var ctrl = this,
		    ...
		    templateUrl: 'template/tabs/c2tabset.html',
...
directives.directive('c2tab', ['$parse', function($parse) {
	  return {
	    require: '^c2tabset',
	    restrict: 'EA',
	    replace: true,
	    templateUrl: 'template/tabs/c2tab.html',
	    transclude: true,
	    scope: {
	      active: '=?',
	      heading: '@',
	    ...
	    controller: function($scope,$element) {
	      //Empty controller so other directives can require being 'under' a tab
	    	//初始化tab右击菜单
	    	this.initMenu = function(menuTarget){
	    		 if(menuTarget){
	    			 $($element).contextmenu({
		        		  target:menuTarget,
		        		  before: function (e, element, target){
		        			  $("div[c2tabset] div[id*='c2-context-menu-']").each(function(){
	    ...
	    compile: function(elm, attrs, transclude) {
	      return function postLink(scope, elm, attrs, c2tabsetCtrl) {
	        scope.$watch('active', function(active) {
	          if (active) {
	            c2tabsetCtrl.select(scope);
	          }
	        });
	        ...
	        c2tabsetCtrl.addTab(scope);
	        scope.$on('$destroy', function() {
	        	c2tabsetCtrl.removeTab(scope);
	        });
...
directives.directive('c2tabHeadingTransclude', ['TabOperator',function(TabOperator) {
	  return {
	    restrict: 'A',
	    require: '^c2tab',
	    link: function(scope, elm, attrs, tabCtrl) {
	      scope.$watch('headingElement', function updateHeadingElement(heading) {
	        if (heading) {
	          ...
	          var menuObj=$(heading).find("div[id*='c2-context-menu-']");
	          if(menuObj.length>0){
	        	  tabCtrl.initMenu(menuObj);
	          }
	          TabOperator.adjust();
	        }
	      });
...
directives.directive('c2tabContentTransclude', function() {
	  return {
		    restrict: 'A',
		    require: '^c2tabset',
		    link: function(scope, elm, attrs) {
		      var tab = scope.$eval(attrs.c2tabContentTransclude);
		  ...
		  function isTabHeading(node) {
		    return node.tagName &&  (
		      node.hasAttribute('c2tab-heading') ||
		      node.hasAttribute('c2data-tab-heading') ||
		      node.tagName.toLowerCase() === 'c2tab-heading' ||
		      node.tagName.toLowerCase() === 'c2data-tab-heading'
		    );
		  }
		});
...
app.config(['$routeProvider', '$httpProvider', '$controllerProvider', function ($routeProvider, $httpProvider, $controllerProvider) {
    $routeProvider.when('/:url*', {controller : 
